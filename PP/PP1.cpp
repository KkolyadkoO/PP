...
void MakePlanar(graph& input) {
	...
	graph inpG = input, inpG1 = input; // Создаём граф внутри функции, из которого будем удалять рёбра в поисках ответа. Оригинальный же затронем только удалением итоговых рёбер.
		...
		if (!Subgraph_K5.empty()) {
			for (int i = 0; i < potentialAnswer.size(); i++) { // В первом цикле пробуем устранить К5
				...
				if (GetSubgraph_K5(t_Subgraph_K5).empty()) {
					...
					DeleteEdge(inpG1, potentialAnswer[i]);
					break;
				}
				else {
					isReadyK5 = false;
	...
	int n = 0;
	vector<int> vertexneedtodelete;// Поиск вершин необходимых для удаления
	for (int i = 0; i < input.Vertices.size(); ++i)
	{
		for (const auto& y : input.Vertices[i].IncidenceList)
		{
			for (int j = needtodelete.size() - 1; j >= 0; j--)
			{
				if (y == needtodelete[j])
				{

					vertexneedtodelete.push_back(i);
	...
	cout << "Minimal list of Vertex to be deleted: ";
	for (int i = 0; i < vertexneedtodelete.size(); i++) {
		cout << vertexneedtodelete[i] + 1 << " ";
		DeleteVertex(input, vertexneedtodelete[i] - n);// Удаление вершин
		n++;
	}
	cout << "(" << vertexneedtodelete.size() << " vertex)\n\n";
}
...
int main() {
	graph inp;
	string filename = { "inputN.txt" };
	string outputname = { "outputN.txt" };
	for (int i = 49; i < 54; i++) {
		filename[5] = (char)i;
		outputname[6] = (char)i;
